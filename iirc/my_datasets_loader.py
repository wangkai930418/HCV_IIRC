...
datasets_names = ["incremental_cifar100", "iirc_cifar100", "incremental_imagenet_full", "incremental_imagenet_lite",
                  "iirc_imagenet_full", "iirc_imagenet_lite", "my_iirc_cifar100"]
...
def get_lifelong_datasets(dataset_name: str,
    ...
    if framework == PYTORCH:
        from iirc.lifelong_dataset.my_torch_dataset import Dataset
...
def _get_dataset(dataset_name: str, dataset_root: str) -> Tuple[Dict[str, DatasetStructType], Dict]:
    ...
    elif dataset_name == "my_iirc_cifar100":
        dataset_configuration = dataset_configuration["my_iirc_cifar100"]

        datasets = \
            _load_my_iirc_cifar100(root=dataset_root,
                                intask_valid_train_ratio=dataset_configuration["intask_valid_train_ratio"],
                                posttask_valid_train_ratio=dataset_configuration["posttask_valid_train_ratio"])
...
def _load_my_iirc_cifar100(root: str = ".", intask_valid_train_ratio: float = 0.1,
                        posttask_valid_train_ratio: float = 0.1) -> Dict[str, DatasetStructType]:
    ...
    with open(os.path.join(metadata_root, "my_iirc_cifar100_hierarchy.json"), "r") as f:
        class_hierarchy = json.load(f)
    ...
    rootclass_to_superclass= class_hierarchy['root_classes']
    superclass_to_rootclass = {superclass: rootclass for rootclass, superclasses in rootclass_to_superclass.items()
    ...
    for subclass in subclasses_to_superclass.keys():
        ...
        for dataset_type in datasets.keys():
            ...
            for idx in samples_idx[dataset_type]:
                ...
                if superclass is None:
                    datasets[dataset_type].append((image, (subclass,)))
                ######################### complement root class label

                elif superclass in superclass_to_rootclass.keys():
                    rootclass=superclass_to_rootclass[superclass]
                    datasets[dataset_type].append((image, (rootclass, superclass, subclass)))
...
def _get_tasks_configuration(dataset_name: str, tasks_configuration_id: int = 0, joint: bool = False) -> Tuple[
    ...
    if dataset_name == "iirc_cifar100":
        ...
        tasks_file = os.path.join(metadata_root, "my_iirc_cifar100_task_configurations.json")
        assert 0 <= tasks_configuration_id <= 0
