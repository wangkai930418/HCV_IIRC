...
from experiments.train import task_train, tasks_eval, task_eval_superclass
...
def main_worker(gpu, config: dict, dist_args: dict = None):
    ...
    # for cur_task_id in range(starting_task, 2):
        ...
        ########################################################################################################
        ########################## test whether could be rightly classified to the superclass
        # tasks_eval(model, lifelong_datasets["test"], \
        #         cur_task_id-1, config, metadata, \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, default="finetune",
                        choices=["finetune", "mask_seen_classes", "lucir", "lucir_noloss3","agem", "icarl_cnn",
                                 "icarl_norm"])
    ################################################################################
