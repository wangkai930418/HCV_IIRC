...
from experiments.ptm_train import task_train, tasks_eval, task_eval_superclass, modified_tasks_eval
...
if __name__ == '__main__':
    ...
    for Threshold in [0.6]:
        acc_list=[str(Threshold),]
        ori_acc_list = [str(Threshold), ]
        file_name='./outputs/threshold_' + \
                    '_'.join((str(int(Threshold * 100)), str(config['run_id']), str(config['method'])))+ \
                    '.txt'
        f = open(file_name, 'w')

        for test_task_id in range(22):
            take_verified_super_key = os.path.join(config['logging_path'], str(test_task_id) + ".pkl")

            with open(take_verified_super_key, 'rb') as g:
                verified_super_key = pkl.load(g)
            ...
            acc_list.append(str(metrics_dict['average_test_modified_jaccard']))
            ori_acc_list.append(str(metrics_dict['average_test_jaccard_sim']))
            original_pred_list, modified_pred_list, GT = \
                            torch.cat(original_pred_list).int().cpu().detach().numpy(),\
                            torch.cat(modified_pred_list).int().cpu().detach().numpy(),\
                                torch.cat(GT).int().cpu().detach().numpy()

        print("\n".join(acc_list), file=f)
        print(acc_list)
        print("\n".join(ori_acc_list), file=f)
        print(ori_acc_list)
