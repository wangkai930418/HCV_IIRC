...
from experiments.ptm_train_icarl import task_train, tasks_eval, task_eval_superclass_in_training
...
def main_worker(gpu, config: dict, dist_args: dict = None):
    ...
    # cp_lifelong_datasets=copy.deepcopy(lifelong_datasets)
    prior=False
    super_class_index=[]
    prev_verified_super_key = []
    NUM_CLASS_PER_TASK=5
    for cur_task_id in range(starting_task, len(tasks)):
    # for cur_task_id in range(starting_task, 4):
        ...
        ####################################### PTM train, deal with labels #####################################
        if cur_task_id > 1:
            # lifelong_datasets['test'].tasks
            # metadata["class_names_to_idx"]
            save_super_class = os.path.join(config['logging_path'], 'super_' + str(cur_task_id - 1) + ".pkl")
            with open(save_super_class, 'rb') as f:
                super_class_index = pkl.load(f)

            take_verified_super_key = os.path.join(config['logging_path'], str(cur_task_id - 1) + ".pkl")
        ...
        if cur_task_id>0:
            # np.where(verified_super_key[0] == 1)[0]
            verified_super_key_dict={}

            verified_super_key,all_keys,all_probs,all_pos_super_key = task_eval_superclass_in_training(
                model, lifelong_datasets["train"], cur_task_id, config, metadata, logbook=logbook,
                dataset_type="valid", dist_args=dist_args,
                Threshold=config['threshold'], num_class_per_task=NUM_CLASS_PER_TASK
            )

            for ind in range(len(verified_super_key)):
                find_where = np.where(verified_super_key[ind][:-NUM_CLASS_PER_TASK] == 1)[0]
                if len(find_where) > 0:
                    if find_where[0] not in super_class_index:
                        super_class_index.append(np.where(verified_super_key[ind][:-NUM_CLASS_PER_TASK] == 1)[0][0])

            if len(prev_verified_super_key) > 0:
                verified_super_key = prev_verified_super_key + verified_super_key

            for super_ind in range(len(verified_super_key)):
                super_label, sub_label=np.where(verified_super_key[super_ind] == 1)[0]
                verified_super_key_dict[sub_label]=super_label

            save_verified_super_key = os.path.join(config['logging_path'], str(cur_task_id) + ".pkl")
            with open(save_verified_super_key, 'wb') as f:
                pkl.dump(verified_super_key, f)

            save_super_class = os.path.join(config['logging_path'], 'super_' + str(cur_task_id) + ".pkl")
            ...
            verified_super_key=verified_super_key.cuda().bool()
        else:
            verified_super_key = None
            verified_super_key_dict=None
        ...
        task_train(model, buffer, lifelong_datasets, config, metadata, logbook=logbook, dist_args=dist_args,
                   verified_super_key_dict=verified_super_key_dict,super_class_index=super_class_index)
...
if __name__ == '__main__':
    ...
    # The training method  ###############################################
    parser.add_argument('--method', type=str, default="icarl_cnn_ptm",)
    ################################################################################
