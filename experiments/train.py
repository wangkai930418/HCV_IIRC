...
def epoch_train(model, dataloader, config, metadata, gpu=None, rank=0,verified_super_key=None,super_class_index=None,sub_class_index=None):
    train_loss = 0
    ...
    for minibatch in dataloader:
        ...
        #             # if (lbl.numpy().astype(np.int32) & verified_super_key[ind])[10:].sum()>0:
        #                 labels[lbl_ind]=torch.from_numpy(verified_super_key[ind]).float()
...
def evaluate(model, dataloader, config, metadata, test_mode=False, gpu=None,verified_super_key=None,super_class_index=None,sub_class_index=None):
    valid_loss = 0
...
def task_train(model, buffer, lifelong_datasets, config, metadata, logbook, dist_args=None,
               verified_super_key=None,super_class_index=None,sub_class_index=None):
    ...
    for epoch in range(config['task_epoch'], num_epochs):
        ...
        train_loss, train_metrics = epoch_train(model, train_loader, config, metadata, gpu, rank,
                                                verified_super_key=verified_super_key,
                                                super_class_index=super_class_index,
                                                sub_class_index=sub_class_index)
        ...
        valid_loss, valid_metrics = evaluate(model, valid_loader, config, metadata, test_mode=False, gpu=gpu,
                                             verified_super_key=verified_super_key,super_class_index=super_class_index,
                                             sub_class_index=sub_class_index)
...
def tasks_eval(model, dataset, cur_task_id, config, metadata, logbook, dataset_type="valid", dist_args=None, verified_super_key=None):
    ...
    _, metrics = evaluate(model, dataloader, config, metadata, test_mode=True, gpu=gpu,verified_super_key=verified_super_key)
...
def eval_superclass(model, dataloader, config, metadata, test_mode=False, gpu=None, Threshold=0.58, num_class_per_task=5):
    ...
    with torch.no_grad():
        ...
        for minibatch in dataloader:
            ...
            if "imagenet_class_names_to_idx" in metadata.keys():
                labels = transform_labels_names_to_vector(
                    labels_names, num_seen_classes, imagenet_class_names_to_idx
                )
                # print(imagenet_class_names_to_idx)
            else:
                labels = transform_labels_names_to_vector(
                    labels_names, num_seen_classes, class_names_to_idx
                )
        ...
        for key in super_pred_dict.keys():
            ...
            for indices in np.argsort(-probs)[:5]:
                print(idx_to_class_names[indices])
            ...
            if prob > Threshold:
                ...
                ######################## rebuttal total wrong , xia jb gao#################
                # pred_superclass=probs.argmax()
                # if len(max_string)>15:
                #     pred_superclass=np.random.randint(low=10,high=len(max_string)-5)
                # else:
                #     pred_superclass=np.random.randint(low=0,high=10)
                # preset_label[pred_superclass] = 1
...
def eval_superclass_in_training(model, dataloader, config, metadata, test_mode=False, gpu=None, Threshold=0.58, num_class_per_task=5):
    valid_loss = 0
    valid_metrics = {'jaccard_sim': 0., 'modified_jaccard': 0., 'strict_acc': 0., 'recall': 0.}
    data_len = 0
    class_names_to_idx = metadata["class_names_to_idx"]
